...
export default function BossRaid({ wallet, theme, isDark }) {
  ...
  // ── Boss icon based on HP ─────────────────────────────────────────────────
  const bossIcon = (pct) => {
    const color = hpColor(pct);
    if (pct > 33) return <TargetIcon size={80} style={{ color }} />;
    return <SkullIcon size={80} style={{ color }} />;
  };
  ...
  if (!isConnected) return (
    <div style={{ padding: "64px 20px", textAlign: "center" }}>
      <SwordIcon size={48} style={{ color: "#ff3b3b", margin: "0 auto 16px" }} />
      <div style={{ color: theme.text, fontSize: "18px", fontWeight: "700", marginBottom: "8px", letterSpacing: "-0.3px" }}>Boss Raid</div>
    ...
    <div style={{ padding: "24px", maxWidth: "800px", margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: "24px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
          <SwordIcon size={18} style={{ color: "#ff3b3b" }} />
      ...
      <div style={{
        ...
        <div style={{
          ...
          {boss ? bossIcon(boss.hpPercent) : <TargetIcon size={80} style={{ color: "#ff3b3b" }} />}
        </div>
        ...
        {/* HP display */}
        <div style={{ color: theme.text, fontSize: "28px", fontWeight: "900", marginBottom: "8px" }}>
          {boss?.currentHP?.toLocaleString()} / {boss?.maxHP?.toLocaleString()} HP
        </div>

        {/* HP bar */}
        <div style={{ background: isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.1)", borderRadius: "12px", height: "16px", margin: "0 0 20px", overflow: "hidden" }}>
          <div style={{
            height:       "100%",
        ...
        {/* Attack info */}
        {!boss?.defeated && (
          <div style={{ color: theme.textMuted, fontSize: "12px", marginTop: "12px" }}>
        ...
        {boss?.defeated && boss?.winner && (
          <div style={{
            ...
            <TrophyIcon size={14} style={{ verticalAlign: "middle", marginRight: 6 }} />Winner: {shortAddr(boss.winner)} landed the killing blow! Next boss is spawning...
          </div>
        )}
      </div>
      ...
      {playerStats && (
        <div style={{
          ...
          <div style={{ color: theme.text, fontWeight: "800", fontSize: "16px", marginBottom: "16px", display: "flex", alignItems: "center", gap: "8px" }}>
            <SwordIcon size={16} style={{ color: "#ff3b3b" }} /> Your Raid Stats
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px,1fr))", gap: "12px" }}>
            {[
              ...
              <div key={s.label} style={{ background: theme.bgCard, border: `1px solid ${theme.border}`, borderRadius: "12px", padding: "14px", textAlign: "center" }}>
                <div style={{ color: s.color, fontWeight: "800", fontSize: "22px" }}>{s.value}</div>
                <div style={{ color: theme.textMuted, fontSize: "11px", marginTop: "4px" }}>{s.label}</div>
              </div>
            ))}
          </div>
          {playerStats.hasAttackedThisRaid && (
            <div style={{ marginTop: "12px", color: "#00c853", fontSize: "13px", fontWeight: "600", textAlign: "center" }}>
              <CheckCircleIcon size={14} style={{ verticalAlign: "middle", marginRight: 4 }} />You have attacked this raid — keep attacking for more chances!
            </div>
          )}
        </div>
      )}
      ...
      <div style={{
        ...
        {recentAttacks.length === 0 ? (
          <div style={{ color: theme.textMuted, fontSize: "14px", textAlign: "center", padding: "20px 0" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {recentAttacks.map((atk, i) => (
              <div key={i} style={{
                background:   atk.killingBlow ? "rgba(240,180,41,0.08)" : (isDark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.03)"),
                border:       `1px solid ${atk.killingBlow ? "rgba(240,180,41,0.3)" : theme.border}`,
                borderRadius: "10px",
                padding:      "10px 14px",
                display:      "flex",
                alignItems:   "center",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <span style={{ display: "flex" }}>
                    {atk.killingBlow
                      ? <SkullIcon size={18} style={{ color: "#f0b429" }} />
                  ...
                  <div>
                    ...
                    <span style={{ color: theme.textMuted, fontSize: "12px" }}>
                      ...
                      {atk.killingBlow && <span style={{ color: "#f0b429", fontWeight: "700" }}> — KILLING BLOW! <TrophyIcon size={12} style={{ verticalAlign: "middle" }} /></span>}
                    </span>
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ color: theme.textMuted, fontSize: "11px" }}>{timeAgo(atk.timestamp)}</div>
                  <div style={{ color: theme.textMuted, fontSize: "11px" }}>{atk.hpAfter.toLocaleString()} HP left</div>
                </div>
              </div>
            ))}
          </div>
